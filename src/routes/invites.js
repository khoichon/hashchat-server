...
const HASH_RE = /^#?[a-zA-Z0-9]{4,12}$/;

// POST /api/invites
router.post('/', inviteLimiter, requireAuth, async (req, res) => {
  try {
    const { toHash, roomId } = req.body;
    if (!toHash || !HASH_RE.test(toHash)) return res.status(400).json({ error: 'invalid hash' });
    ...
    const [{ data: sender }, { data: room }] = await Promise.all([
      supabaseAdmin.from('users').select('name,hash,color').eq('id', fromUserId).maybeSingle(),
...
router.post('/:id/accept', requireAuth, async (req, res) => {
  try {
    ...
    const { data: invite } = await supabaseAdmin
      .from('invites').select('*').eq('id', req.params.id).eq('to_user_id', req.user.id).maybeSingle();
    ...
    await supabaseAdmin.from('room_members')
      .upsert({ room_id: invite.room_id, user_id: req.user.id }, { onConflict: 'room_id,user_id' });
    await supabaseAdmin.from('invites').update({ status: 'accepted' }).eq('id', req.params.id);
...
router.post('/:id/decline', requireAuth, async (req, res) => {
  try {
    ...
    await supabaseAdmin.from('invites').update({ status: 'declined' }).eq('id', req.params.id);
    res.json({ ok: true });
  } catch (err) {
...
// POST /api/invites/dm
router.post('/dm', inviteLimiter, requireAuth, async (req, res) => {
  try {
